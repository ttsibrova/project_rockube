...
class ROCKUBE_API URockubeMovementComponent : public UCharacterMovementComponent
{
    ...
    UPROPERTY (EditDefaultsOnly) UCurveFloat* DashMovementCurve;
    ...
    UPROPERTY (EditDefaultsOnly) UCurveFloat* JumpPadMovementCurve;
    UPROPERTY (EditDefaultsOnly) float JumpPadJumpRelativeTime = 0.9;

    FVector JumpPadJumpStartLocationPos;
    FVector JumpPadJumpTargetPoint;
    float JumpPadJumpSimulationTime = -1.f;
    float JumpPadJumpStartTime;
...
private:
    ...
    // JumpPad jump
private:
    void EnterJumpPadJump (EMovementMode PrevMode, ECustomMovementMode PreviousCustomMode);
    void ExitJumpPadJump();
    bool CanJumpPadJump() const;
    void PhysJumpPadJump (float deltaTime, int32 Iterations);
